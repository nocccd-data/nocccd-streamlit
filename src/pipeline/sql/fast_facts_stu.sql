WITH
    sudc AS (
        SELECT DISTINCT
            ...
            'Y' AS econ_disa_ind
        FROM sgbuser
        WHERE SUBSTR(sgbuser_sudc_code, 1, 1) IN ('1', '2', '3', '4')
    ),
        ...
        SELECT
            ...
            LEAD(svbsgpd_eff_term, 1, '999999')
                 OVER (
                     ...
                     ORDER BY svbsgpd_eff_term ASC NULLS LAST
                     ) AS to_term,
            CASE
    ...
    base AS (
        SELECT
            ...
            CASE WHEN s.econ_disa_ind = 'Y' THEN 'Y' ELSE 'N' END AS econ_disa_ind,
            CASE WHEN f.first_gen_ind = 'Y' THEN 'Y' ELSE 'N' END AS first_gen_ind,
            (CASE
        ...
        FROM sfrstcr r
            LEFT JOIN sudc s
                ON (
                r.sfrstcr_pidm = s.sgbuser_pidm
                    AND r.sfrstcr_term_code = s.sgbuser_term_code
                )
