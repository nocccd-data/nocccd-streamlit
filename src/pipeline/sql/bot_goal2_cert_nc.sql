WITH
    ...
    base AS (
        ...
        FROM sfrstcr r
            ...
            LEFT JOIN ssbsect z
                ON (r.sfrstcr_crn = z.ssbsect_crn
                AND r.sfrstcr_term_code = z.ssbsect_term_code
                AND SUBSTR(r.sfrstcr_camp_code, 1, 1) = SUBSTR(z.ssbsect_camp_code, 1, 1))
        WHERE t.stvterm_acyr_code = :acyr_code
          AND (SUBSTR(r.sfrstcr_camp_code, 1, 1) = '3' AND v.stvrsts_apport_ind = 'Y')
          AND (fz_get_course_division(z.ssbsect_subj_code, z.ssbsect_crse_numb, z.ssbsect_term_code)
            IN ('VBMT', 'VMDC', 'CPLB', 'COMP', 'VECE', 'VELE', 'VFSR', 'VMED', 'VPHM', 'VMDA', 'VBOT', 'WFPR', 'PARN',
                'VBSK', 'VCIS', 'VBUS', 'CPAA', 'VCON')
            OR z.ssbsect_subj_code LIKE 'IHS%'
            OR z.ssbsect_subj_code LIKE 'ABE%'
                AND fz_get_course_division(z.ssbsect_subj_code, z.ssbsect_crse_numb,
                                           z.ssbsect_term_code) NOT IN ('EMER', 'OAP', 'LEAP')
            OR ssbsect_subj_code = 'ESLA')
    )
