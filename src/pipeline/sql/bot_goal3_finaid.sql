WITH
    ...
    fin_aid AS (
        SELECT
            a.tbraccd_pidm AS pidm,
            SUBSTR(a.tbraccd_detail_code, 1, 1) AS camp_code,
            t.stvterm_acyr_code AS acyr_code,
            SUM(a.tbraccd_amount) AS sum_amt
        FROM taismgr.tbraccd a
            INNER JOIN saturn.stvterm t
                ON (a.tbraccd_term_code = t.stvterm_code)
        WHERE t.stvterm_acyr_code = :acyr_code
          AND (a.tbraccd_detail_code LIKE '%PEL%'
        ...
        GROUP BY
            a.tbraccd_pidm,
            SUBSTR(a.tbraccd_detail_code, 1, 1),
            t.stvterm_acyr_code
    ),
...
SELECT
    ...
    a.age
FROM base a
    INNER JOIN fin_aid f
        ON (a.pidm = f.pidm
        AND a.camp_code = f.camp_code
        AND a.acyr_code = f.acyr_code)
WHERE a.site = 'Credit'