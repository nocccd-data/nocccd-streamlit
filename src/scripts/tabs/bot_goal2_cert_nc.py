...
from src.scripts.data_provider import fetch_bot_goal2_cert_nc
...
def render():
    ...
    if query_btn:
        ...
        df = fetch_bot_goal2_cert_nc(tuple(sorted(selected_acyrs)))
        if df.empty:
    ...
    render_bot_charts(st.session_state["bg2nc_df"], _TITLES)
