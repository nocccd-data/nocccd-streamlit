...
def read_config(section="dwh"):
    # Prefer Streamlit secrets (for cloud), fall back to config.ini (for local)
    try:
        import streamlit as st
        sec = st.secrets[section]
        return sec["username"], sec["password"], sec["dsn"]
    except Exception:
        pass
...
def init_oracle_client():
    try:
        ...
        if tns:
            if Path(tns).exists():
                logging.info(f"ℹ️ Using TNS_ADMIN={tns} for network/admin files")
            else:
                logging.warning(f"⚠️ TNS_ADMIN is set to {tns} but the path does not exist")

        if ORACLE_CLIENT_PATH:
            oracledb.init_oracle_client(lib_dir=ORACLE_CLIENT_PATH)
            logging.info(f"✅ Oracle Client Loaded in Thick Mode from {ORACLE_CLIENT_PATH}")
        else:
            logging.info("ℹ️ No Oracle client path resolved from ORA_HOME/ORACLE_HOME; attempting thin mode (no lib_dir).")
