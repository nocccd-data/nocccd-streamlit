...
**No era adjustment exists, and the note says so.** `_PROJECTION_NOTE_LINES` is defined once and rendered both as an always-visible caption on screen and as a two-line header on **every** PDF campus page — not only the methodology page, because a single chart page pasted into a deck leaves that page behind and would then carry nothing about what the dashed line was fitted on. The axes give up ~0.3in on pages that show it.

It states plainly that 2020–21 are included as ordinary years, rather than warning about COVID, because the measured case for a COVID adjustment is weak: dropping both cohorts moves the credit-college forecast by ~1 pp and makes Cypress's fit *worse* (R² **0.61 → 0.02**), since those years sit on the trend rather than off it. Weighted Moving Average is unaffected either way — it reads only the last 3 completed cohorts, so COVID is already outside its window. NOCE is the one genuine exception (spring 79% in 2020, next-fall 36% in 2021) and its own R² near 0 already reports "no trend here".
