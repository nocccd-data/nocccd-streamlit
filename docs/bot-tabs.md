...
- **Campus target ticks (optional).** A "Show campus targets" switch (`render_campus_target_toggle`, session key `<prefix>_campus_targets`) sits at the top of each target tab, shown only once the plan frame is loaded; it is **off by default**, so the campus chart looks exactly as it always has. When on, `campus_target_rows` gives each campus its own target for every plan year after the baseline (no tick in 2022-23, where target = actual), and `add_campus_target_ticks` (Plotly) / `mpl_campus_bar_labels` (PDF) draw a thin dark bar exactly as wide as each campus bar at its target, with the number centred under it (white halo, so it reads on NOCE navy). Where the target is above the bar, the actual's label is lifted above the tick so the two never overlap — the only change to the existing chart, and only while the switch is on. The tab's **Download PDF** follows the switch (its cache key includes it); the bulk PDF never sets `show_campus_targets`, so the Board packet is unchanged. The Units tab draws the same ticks at its reduction targets (1 decimal).
