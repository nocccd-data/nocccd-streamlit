...
- **Targets are counts, never drawn on the rate charts.** Converting a count target into a rate target reverses Met/Not Met whenever enrollment moves (AA Hispanic 2025-26: count 1,117 vs target 1,047 is met, but rate 4.01% vs 4.68% is not). Target columns therefore sit next to the counts in Excel, and the only chart markers are the optional campus ticks below (the campus chart plots counts / averages, not rates).
- **Campus target ticks (optional).** A "Show campus targets" switch (`render_campus_target_toggle`, session key `<prefix>_campus_targets`) sits right above the campus chart it controls (drawn by `render_bot_charts` via `campus_toggle_prefix`, or by the Units tab's own render), shown only once the plan frame is loaded; it is **off by default**, so the campus chart looks exactly as it always has. When on, `campus_target_rows` gives each campus its own target for every plan year after the baseline (no tick in 2022-23, where target = actual), and `add_campus_target_ticks` (Plotly) / `mpl_campus_bar_labels` (PDF) draw a thin dark bar exactly as wide as each campus bar at its target, with the number centred under it (white halo, so it reads on NOCE navy). Where the target is above the bar, the actual's label is lifted above the tick so the two never overlap — the only change to the existing chart, and only while the switch is on. The tab's **Download PDF** follows the switch — the PDF is built earlier in the script than the switch is drawn, so it reads the switch's saved state (`campus_targets_on`) and includes it in the cache key; the bulk PDF never sets `show_campus_targets`, so the Board packet is unchanged. The Units tab draws the same ticks at its reduction targets (1 decimal).
- **Campus Excel table**: the headcount (Units: average) by campus table carries a `{year} Target` column right after every plan year's column — 2022-23 through the last displayed year, never before the baseline or past 2029-30 (`campus_target_cols` / `interleave_campus_targets`). Always present, independent of the switch.
